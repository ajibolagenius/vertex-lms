...
- `app/courses/[slug]/page.tsx` — frame (`bg-hatch` + `max-w-[1440px] border-x bg-paper`),
  ...
  with the vertical connector, and lesson rows linking with `lesson.slug`.
...
1. **Route** `app/lessons/[slug]/page.tsx`, `generateStaticParams` over `LESSON_SLUGS_QUERY`,
  `generateMetadata` from the lesson. Course lesson rows and both "Continue Learning" CTAs
  use `lesson.slug` to link here.
...
| `app/courses/[slug]/page.tsx` | lesson rows and CTAs use `lesson.slug` |
