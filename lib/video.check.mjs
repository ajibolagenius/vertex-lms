...
assert.equal(youtubeId("https://music.youtube.com/watch?v=9602Yzvd7ik"), "9602Yzvd7ik");
assert.equal(youtubeId("https://www.youtube-nocookie.com/embed/9602Yzvd7ik"), "9602Yzvd7ik");
assert.equal(youtubeId("https://vimeo.com/12345"), null);
// Near-match hosts: a suffix check alone would let these through.
assert.equal(youtubeId("https://notyoutube.com/watch?v=9602Yzvd7ik"), null);
assert.equal(youtubeId("https://evil-youtube-nocookie.com/embed/9602Yzvd7ik"), null);
assert.equal(youtubeId("not a url"), null);
assert.equal(youtubeId(null), null);

assert.equal(
  youtubeEmbedUrl("abc123", 125),
  "https://www.youtube-nocookie.com/embed/abc123?autoplay=1&rel=0&enablejsapi=1&start=125",
);
assert.equal(
  youtubeEmbedUrl("abc123"),
  "https://www.youtube-nocookie.com/embed/abc123?autoplay=1&rel=0&enablejsapi=1",
);
